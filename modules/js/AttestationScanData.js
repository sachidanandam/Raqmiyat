...
{	
	try
	{
		...
		//scanval="BB48343D";
		var inputparam={serviceID: "GetStudentDetails",ChipSN:scanval,Username:"WSMOHE",Password:"Mohe@Webservice"}
...
gblFlagToAssignEmiratesId=false;gblFlagToAssignMobileCode=false;
//Nithya & Deepa code
function assignCertificateData(Response)
{
			...
			try
			{
			    ...
			    else
			    {
					...
					//-----------------------------------
					frmAttestGrad2.dropdowninstitue.masterDataMap=[[{"key":instituteid,"value":institu}],"key","value"];
					frmAttestGrad2.dropdowninstitue.selectedKey=instituteid;
					frmAttestGrad2.dropdowninstitue.setEnabled(false);	
					//
					frmAttestGrad2.dropdownstudylevel.masterDataMap=[[{"key":gblStudyLevelTemp[levels.toUpperCase()],"value":levels.toUpperCase()}],"key","value"];
					...
					//
					frmAttestGrad3.dropdownstudylevel.masterDataMap=[[{"key":previousstylevelid,"value":previousstudylvl}],"key","value"];
