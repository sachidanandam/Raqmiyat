...
function validateCertAttestPgFieldGoFurther()			//attestation step1 validations
{
	...
	switch(curFrm["id"])
	{
		...
		else if(curFrm.rbimgMale.src=="unchecked.png"&&curFrm.rbimgFemale.src=="unchecked.png")
		{
	...
	case "frmAttestGrad2":
		...
		if(curFrm.dropdowninstitue.selectedKey == "SI")
		{
	 	...
	 	else if(curFrm.txtnoofdocuments!=undefined && curFrm.txtnoofdocuments.text == "" || (kony.os.toNumber(curFrm.txtnoofdocuments.text))==0)
	 	{
	 		alertDialog(kony.i18n.getLocalizedString("lblPleaseEnterNoOfDocuments"));
	...
	*/
		...
		if(selectedCategory == category_Graduate)
		{
			...
			else if(curFrm.txtinstitutename2!=undefined && curFrm.txtinstitutename2.text == "")
			{
				alertDialog(kony.i18n.getLocalizedString("lblPleaseEnterInstitute"));
			...
			//Commented because of client request
...
function checkMobileCode(text) {
    var status = false;
    var mobcode = ["050", "052", "055", "056"]
    for (var i = 0; i < mobcode.length; i++) {
        if (text.toString() == mobcode[i]) {
            status = true;
        }
    }
    return status;
