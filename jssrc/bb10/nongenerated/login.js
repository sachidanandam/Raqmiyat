        ...
        if (status == 400) {
            if (kony.os.toNumber(result.opstatus) == 0) {
                ...
                if (result["resultNew"] == "true" || result["resultNew"] == true) {
                    ...
                    if (isSaveAttachNewUser == true && isSendingImageNewUser == true) saveAttachmetNewUser();
                    else {
                        dismissLoadingIndicator();
        ...
        if (status == 400) {
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                ...
                if (result["outputCountry"] == "SUCCESS" && result["output"] == "SUCCESS") {
                    ...
                    comboboxMobCodesData.push({
                        "mykey": "0",
                        "myvalue": kony.i18n.getLocalizedString("lblSelectAreaCode")
                    });
                    for (var i = 1; i < result["mobCodes"].length; i++) {
                        var key = result["mobCodes"][i]["mobileCode"];
                        var value = result["mobCodes"][i]["mobileCode"];
                        comboboxMobCodesData.push({
                            "mykey": key,
...
function getStudyLevels() {
    ...
    try {
        ...
        if (kony.application.getCurrentForm().dropdowninstitue.selectedKey == "SI") {
            ...
            kony.application.getCurrentForm().dropdownstudylevel.masterDataMap = [
                [{
                    "mykey": "0",
                    "myvalue": ""
                }], "mykey", "myvalue"];
            kony.application.getCurrentForm().dropdownmajor.masterDataMap = [
                [{
                    "mykey": "0",
                    "myvalue": ""
                }], "mykey", "myvalue"];; //[comboboxEmptyData,"mykey","myvalue"];
            kony.application.getCurrentForm().dropdownSubMajor.masterDataMap = [
                [{
                    "mykey": "0",
                    "myvalue": ""
                }], "mykey", "myvalue"]; //[comboboxEmptyData,"mykey","myvalue"];
    ...
    if (kony.net.isNetworkAvailable(constants.NETWORK_TYPE_ANY)) {
        try {
            ...
            if (kony.application.getCurrentForm().dropdownstudylevel.selectedKey == 0) {
                ...
                kony.application.getCurrentForm().dropdownmajor.masterData = [
                    [{
                        "mykey": "1",
                        "myvalue": ""
                    }], "mykey", "myvalue"]; //[comboboxEmptyData,"mykey","myvalue"];
...
function getSubMajor() {
    try {
        ...
        if (kony.application.getCurrentForm().dropdownmajor.selectedKey == 0) {
            kony.application.getCurrentForm().dropdownSubMajor.setEnabled(true);
            kony.application.getCurrentForm().dropdownSubMajor.masterData = [
                [{
                    "mykey": "0",
                    "myvalue": ""
                }], "mykey", "myvalue"]; //[comboboxEmptyData,"mykey","myvalue"];
...
function createAttestGrad2Obj() {
    ...
    if (selectedCategory == category_Enrolled) attesGradObj["subMajor"] = frmAttestGrad2.dropdownSubMajor.selectedKey;
...
function handleBrowserCallback(browserWidget, params) {
    ...
    if (paymentURL == params["originalURL"]) {
    ...
    try {
        if (status == 400) {
            if ((kony.os.toNumber(resultTable.opstatus) == 0)) {
                ...
                } else kony.print("no modification found on properties file..")
            }
        }
    } catch (err) {
        displayError(err);
