...
function validateCertEquiPgFieldGoFurther() {
    if (kony.net.isNetworkAvailable(constants.NETWORK_TYPE_ANY)) {
        ...
        case "frmCertEquivalency4":
            ...
            if (curFrm.segPreviousCertificate.data == null || curFrm.segPreviousCertificate.data == "" || curFrm.segPreviousCertificate.data.length <= 0) {
...
function certEquiAddTransferCredit() {
    ...
    if (curFrm.btnTransfAdd.text != "Add" && curFrm.segTransferCredit.selectedIndex != null && curFrm.segTransferCredit.selectedIndex != "") {
        ...
        var addSegment1 = {
            lblTransCertTitle: "University : " + university,
            lblTransCertCountry: "Country : " + country,
            lblTransCertYears: "No of years : " + years,
...
function certEquiAddPreviousCertificate() {
    ...
    if (curFrm.btnPreCertAdd.text != "Add" && curFrm.segPreviousCertificate.selectedIndex != null && curFrm.segPreviousCertificate.selectedIndex != "") {
        ...
        var addPrevious = {
            ...
            lblUniversitySegDisplay: "University : " + univ,
            lblCertificateNameSegDisplay: "Certificate : " + certificate,
            lblCountryYearSegDisplay: "Country : " + country,
            lblPrevCertYear: "Period From : " + from + " to : " + to,
...
function invokeCertEquiWs() {
    ...
    else if (isGenderMaleSerVal != (curFrm.rbimgmale.src = "checked.png" ? true : false)) invokeEquiWsUpdateApp();
        ...
        if (status == 400) {
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                ...
                if (result["outPut"] != null && result["outPut"].trim().toUpperCase() == "SUCCESS") {
                    if (result["appNo"] != null && result["appNo"] != "") gvAppNo = result["appNo"];
...
function invokeEquiWsInsertPreCert() {
    ...
    if (gvPreTranCretCnt == null || gvPreTranCretCnt == "") if (segData == null || segData == "" || segData.length <= 0) {
        gvPreTranCretCnt = null;
        invokeEquiWsInsertTranCert();
        return;
    } else {
...
function invokeEquiWsUploadCert() {
	...
	return;
		*/
    var segData;
    if (gblDeviceInfo == "tab") segData = frmCertEquivalencyTab2.segAttachment.data;
    else segData = frmCerEquivalencyAttach.segAttachment.data;
    if (gvPreTranCretCnt == null || gvPreTranCretCnt == "") if (segData == null || segData == "" || segData.length <= 0) {
        gblAppNoFromInsertApp = gvAppNo;
        resetCertificateEquivalencytab();
        frmCertEnquApplicationSubmit.show();
	...
	inputparam["ODT"] = inputXml;*/
    inputparam["APP_NO"] = gvAppNo;
    inputparam["FILENAME"] = segData[gvPreTranCretCnt]["lblName"];
    inputparam["DESCRIPTION"] = segData[gvPreTranCretCnt]["hdnIndex"];
    inputparam["Content"] = binaryFile;
        ...
        if (status == 400) {
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                ...
                if (result["outputCountry"] == "SUCCESS" && result["outputstudylevel"] == "SUCCESS" && result["output"] == "SUCCESS") {
                    ...
                    emiratesListData = [{
                        "mykey": "0",
                        "myvalue": kony.i18n.getLocalizedString("lblSelectEmirate")
                    }];
        ...
        if ((kony.os.toNumber(result.opstatus) == 0)) {
            ...
            if (transcertdetails != null && transcertdetails != "") {
                ...
                for (var i = 0; i < transcertdetails.length; i++) {
                    if (transcertdetails[i]["CERT_NO"] == null || transcertdetails[i]["CERT_NO"] == "" || transcertdetails[i]["CERT_NO"] != gvCretNo) continue;
