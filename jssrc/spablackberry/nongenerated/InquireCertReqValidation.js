        ...
        if (status == 400) {
            ...
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                ...
                if (result["Status"] == "SUCCESS" && result["DegreeApplicationID"] > 0) {
                    ...
                    showErrMsgWithHandle(kony.i18n.getLocalizedString("msgApplicationSuccess") + DegreeApplicantID, goToManiMenu); // , invokeUploadFileRequest(frmName,applicantno));
                } else {
                    showErrMsgWithHandle(result["Status"], goToManiMenu);
                }
            } else {
                dismissLoadingIndicator();
...
function invokeUploadFileRequest(frmName, AppNo) {
    ...
    try {
        showLoadingIndicator(kony.i18n.getLocalizedString("lblProcessingReq"));
        var filename = frmName.segAttachment.selectedItems[0]["lblFileName"]; //(gblDeviceInfo == "tab") ? frmCertRequest.segAttachment.selectedItems[0]["lblFileName"] : frmCertRequest1.segAttachment.selectedItems[0]["lblFileName"];
        if (deviceName == "android") {
            var filepath = frmName.segAttachment.selectedItems[0]["hdPath"]; //(gblDeviceInfo == "tab") ? frmCertRequest.segAttachment.selectedItems[0]["hdPath"] : frmCertRequest1.segAttachment.selectedItems[0]["hdPath"];
            returnedByteData = FileExplorer.convertIntoByte(filepath);
            kony.print(returnedByteData);
        } else {
            //returnedByteData = FileExplorer.convertUIImageIntoBinaryArray();
        }
        var inputfileparam = {
        ...
        if (status == 400) {
            ...
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                if (result["output"] == "SUCCESS" && result["result"] == "true") {
                    showErrMsgWithHandle(kony.i18n.getLocalizedString("msgFileUploadSuccess"), goToManiMenu);
                } else {
                    showErrMsgWithHandle(result["output"], goToManiMenu);
                }
            } else {
                showErrMsgWithHandle(kony.i18n.getLocalizedString("loadingIndicator"), goToManiMenu);
            }
