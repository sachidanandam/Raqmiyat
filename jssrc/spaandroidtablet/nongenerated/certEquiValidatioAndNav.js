...
function validateCertEquiPgFieldGoFurther() {
    ...
    case "frmCertEquivalency2":
        if (curFrm.cmbTypeOfStudy.selectedKey == 0) {
            ...
            return false;
        } else if (curFrm.txtNoofyears = undefined || curFrm.txtNoofyears.text == "") {
        ...
        /*else if (curFrm.txtNoofTerms = undefined || curFrm.txtNoofTerms.text == "")
            ...
            if (gblDeviceInfo != "tab") frmCertEquivalency3.show();
            else frmCertEquivalencyTab2.show();
        }
        break;
    ...
    case "frmCertEquivalency4":
        ...
        if (curFrm.segPreviousCertificate.data == null || curFrm.segPreviousCertificate.data == "") {
            alertDialog("Please Add previous certificate details");
            return false;
        } else {
            if (gblDeviceInfo != "tab") {
    ...
    case "frmCerEquivalencyAttach":
        if (curFrm.segAttachment.data == null || curFrm.segAttachment.data == "") {
            alertDialog("Please Add certificate soft copy");
            return false;
        } else invokeCertEquiWs();
        break;
    }
}
...
function certEquiAddTransferCredit() {
    var curFrm;
    curFrm = kony.application.getCurrentForm();
    if (gblDeviceInfo == "tab") curFrm.hboxSeg1.setVisibility(true);
    ...
    if (curFrm.btnTransfAdd.text != "Add" && curFrm.segTransferCredit.selectedIndex != null && curFrm.segTransferCredit.selectedIndex != "") {
        ...
        var addSegment1 = {
            lblTransCertTitle: {
                text: university + ", " + country + ", " + years + ".test"
            },
    ...
    curFrm.btnTransfAdd.text = "Add";
}

function certEquiDeleteTransferCreditRow() {
    var curFrm;
    curFrm = kony.application.getCurrentForm();
    if (curFrm.segTransferCredit.selectedItems[0]["hdTranCretNo"] != null && curFrm.segTransferCredit.selectedItems[0]["hdTranCretNo"] != "") alertDialog(kony.i18n.getLocalizedString("errMsgCantDeletePreSubCret"));
    else showConfirmationMsgWithHandle("removeConfMsg", certEquiDeleteTransferCreditRowAlertCb, "lblYes", "lblNo");
}
...
function certEquiAddPreviousCertificate() {
    var curFrm;
    curFrm = kony.application.getCurrentForm();
    if (gblDeviceInfo == "tab") curFrm.hboxSeg2.setVisibility(true);
    if (curFrm.dropdownlevel.selectedKey == 0) {
        ...
        return false;
    } else if (!validateYear(curFrm.txtPeriodFrom.text, "please enter valid year")) {
        alertDialog(kony.i18n.getLocalizedString("lblPleaseEnterValidYearVaule"));
        gblErrWidgetRefId = curFrm.txtPeriodFrom;
        return false;
    ...
    var prevCretNo = "";
    if (curFrm.btnPreCertAdd.text != "Add" && curFrm.segPreviousCertificate.selectedIndex != null && curFrm.segPreviousCertificate.selectedIndex != "") {
        ...
        var addPrevious = {
            ...
            lblCountryYearDisplay: country + ", " + from + "-" + to,
    ...
    curFrm.btnPreCertAdd.text = "Add";
}

function certEquiDeletePreviousCertificateRow() {
    var curFrm;
    curFrm = kony.application.getCurrentForm();
    if (curFrm.segPreviousCertificate.selectedItems[0]["hdPrevCretNo"] != null && curFrm.segPreviousCertificate.selectedItems[0]["hdPrevCretNo"] != "") alertDialog(kony.i18n.getLocalizedString("errMsgCantDeletePreSubCret"));
    else showConfirmationMsgWithHandle("removeConfMsg", certEquiDeletePreviousCertificateRowAlertCb, "lblYes", "lblNo");
}
...
function invokeCertEquiWs() {
    ...
    if (gvAppNo == null || gvAppNo == "") invokeEquiWsInsertApp();
    else invokeEquiWsInsertCert();
}
    ...
    try {
        if (status == 400) {
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                ...
                if (result["appNo"] != null && result["appNo"] != "" && result["outPut"] != null && result["outPut"].trim().toUpperCase() == "SUCCESS") {
...
function invokeEquiWsInsertCert() {
    var inputparam = [];
    if (isNewCertReg == true) inputparam["serviceID"] = "Equ_Insert_Cert";
    else inputparam["serviceID"] = "Equ_Update_Cert";
    inputparam["httpheaders"] = {};
    inputparam["httpconfig"] = {
        timeout: 5
    };
    ...
    if (isNewCertReg != true) inputparam["CERT_NO"] = gvCretNo;
    var curFrm;
    if (gblDeviceInfo == "tab") curFrm = frmCertEquivalencyTab1;
    else curFrm = frmCertEquivalency1;
    ...
    if (gblDeviceInfo != "tab") curFrm = frmCertEquivalency2;
        ...
        if (status == 400) {
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                ...
                if (result["outPut"] != null && result["outPut"].trim().toUpperCase() == "SUCCESS") {
                    if (result["CERT_NO"] != null && result["CERT_NO"] != "" && isNewCertReg == true) gvCretNo = result["CERT_NO"];
...
function invokeEquiWsInsertPreCert() {
    var segData;
    if (gblDeviceInfo == "tab") segData = frmCertEquivalencyTab2.segPreviousCertificate.data;
    else segData = frmCertEquivalency4.segPreviousCertificate.data;
    if (gvPreTranCretCnt == null || gvPreTranCretCnt == "") if (segData == null || segData == "" || segData.length <= 0) invokeEquiWsInsertTranCert();
    else gvPreTranCretCnt = segData.length;
    if (isNewCertReg == true) gvPreTranCretCnt--;
    else {
        skipUnchangePreCertRecord: do {
            ...
            if (segData[gvPreTranCretCnt]["hdPrevCretNo"] != "" && segData[gvPreTranCretCnt]["hdIsUpdate"] != true) continue skipUnchangePreCertRecord;
            else break;
        } while (1);
    }
    if (gvPreTranCretCnt < 0) {
    ...
    var inputparam = [];
    if (isNewCertReg == true || (isNewCertReg != true && (segData[gvPreTranCretCnt]["hdPrevCretNo"] == null || segData[gvPreTranCretCnt]["hdPrevCretNo"] == ""))) inputparam["serviceID"] = "Equ_Insert_PreCert";
    else {
    ...
    try {
        if (status == 400) {
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                ...
                if (result["outPut"] != null && result["outPut"].trim().toUpperCase() == "SUCCESS") {
                    if (result["prevcertno"] != "" && result["prevcertno"] != null) gvPreCretNo = result["prevcertno"] + "~";
                    if (gvPreTranCretCnt > 0) invokeEquiWsInsertPreCert();
                    else {
...
function invokeEquiWsInsertTranCert() {
    var segData;
    if (gblDeviceInfo == "tab") segData = frmCertEquivalencyTab2.segTransferCredit.data;
    else segData = frmCertEquivalency3.segTransferCredit.data;
    if (gvPreTranCretCnt == null || gvPreTranCretCnt == "") if (segData == null || segData == "" || segData.length <= 0) invokeEquiWsUploadCert();
    else gvPreTranCretCnt = segData.length;
    if (isNewCertReg == true) gvPreTranCretCnt--;
    else {
        skipUnchangeTranCertRecord: do {
            ...
            if (segData[gvPreTranCretCnt]["hdTranCretNo"] != "" && segData[gvPreTranCretCnt]["hdIsUpdate"] != true) continue skipUnchangeTranCertRecord;
            else break;
        } while (1);
    }
    if (gvPreTranCretCnt < 0) invokeEquiWsUploadCert();
    var inputparam = [];
    //if(isNewCertReg == true)
    if (isNewCertReg == true || (isNewCertReg != true && (segData[gvPreTranCretCnt]["hdTranCretNo"] == null || segData[gvPreTranCretCnt]["hdTranCretNo"] == ""))) inputparam["serviceID"] = "Equ_Insert_TransCert";
    else {
        inputparam["serviceID"] = "Equ_Update_PreCert";
    ...
    try {
        if (status == 400) {
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                kony.print("result data is------------", result);
                if (result["outPut"] != null && result["outPut"].trim().toUpperCase() == "SUCCESS") {
                    if (result["TRANCERT_NO"] != "" && result["TRANCERT_NO"] != null) gvTranNo = result["TRANCERT_NO"];
                    if (gvPreTranCretCnt > 0) invokeEquiWsInsertTranCert();
                    else {
                        gvPreTranCretCnt = "";
                        invokeEquiWsUploadCert();
                    }
...
function invokeEquiWsUploadCert() {
    ...
    dismissLoadingIndicator();
    return;
    var segData = frmCertEquivalency2.segCertificateAttachment.data;
    if (gvPreTranCretCnt == null || gvPreTranCretCnt == "") if (segData == null || segData == "" || segData.length <= 0) { /* Todo Need to Add  Code */
    } else gvPreTranCretCnt = segData.length;
    gvPreTranCretCnt--;
    if (gvPreTranCretCnt < 0) { /* Todo Need to Add  Code */
        gvPreTranCretCnt = "";
    }
    var inputparam = [];
    inputparam["serviceID"] = "Equ_UploadFile";
    inputparam["httpheaders"] = {};
    inputparam["httpconfig"] = {
        timeout: 5
    };
    ...
    inputparam["wsPwd"] = gvWsPwd;
    var binaryFile;
    /*  Todo: Need to implement native code */
    /*
	var	inputXml = "";	
		inputXml = inputXml + "<APP_NO>" + gvAppNo + "</APP_NO>";
	...
	inputparam["ODT"] = inputXml;*/
    inputparam["APP_NO"] = gvAppNo;
    inputparam["FILENAME"] = segData[gvPreTranCretCnt]["lblFileName"];
    inputparam["DESCRIPTION"] = segData[gvPreTranCretCnt]["hdnFileIndex"];
    inputparam["FILEDATA"] = binaryFile;
...
function equiWsUploadCallback(status, result) {
    try {
        if (status == 400) {
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                kony.print("result data is------------", result);
                if (result["UploadFileResult"] == "true" || result["UploadFileResult"] == true) {
                    if (gvPreTranCretCnt > 0) {
...
function resetCertificateEquivalency() {
    ...
    frmCertEquivalency1.rbimgPublic.src = "checked.png";
    ...
    frmCertEquivalency2.imgSelf.src = "checked.png";
...
function resetCertificateEquivalencytab() {
    ...
    frmCertEquivalencyTab1.imgSelf.src = "checked.png";
...
function loadequivalencycomposite() {
    try {
        if ((gblDeviceInfo == "tab" && kony.application.getCurrentForm() == frmCertEquivalencyTab2) || (gblDeviceInfo != "tab" && (kony.application.getCurrentForm() == frmCertEquivalency2 || kony.application.getPreviousForm() == frmCertEquivalency2))) return;
        showLoadingIndicator(kony.i18n.getLocalizedString("lblPleaseWait"));;
        //gvAppNo = "201412160";
        var inputparam = [];
        ...
        if (status == 400) {
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                ...
                if (result["outputCountry"] == "SUCCESS" && result["outputstudylevel"] == "SUCCESS" && result["output"] == "SUCCESS") {
                    ...
                    comboboxData = [{
                        "mykey": "0",
                        "myvalue": kony.i18n.getLocalizedString("lblSelectCountry")
                    }];
                    ...
                    comboboxStudyLevelData = [{
                        "mykey": "0",
                        "myvalue": "Please Select Study Level"
                    }];
                    for (var i = 0; i < result["studyLevelList"].length; i++) {
                        ...
                        comboboxStudyLevelData.push({
                            "mykey": key,
                            "myvalue": value
                        })
                    }
                    var comboboxPaymentMethodTypeData = [];
                    comboboxPaymentMethodTypeData = [{
                        "mykey": "SPMT",
                    ...
                    for (var i = 0; i < result["paymentsList"].length; i++) {
                        ...
                        comboboxPaymentMethodTypeData.push({
                            "mykey": key,
                            "myvalue": value
                        })
                    }
                    ...
                    if (isNewCertReg != true) assignRegCretRelatedDetails(result);
        ...
        if (status == 400) {
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                ...
                if (result["output"] == "SUCCESS") {
                    ...
                    comboboxData = [{
                        "mykey": "0",
                        "myvalue": "Please select payment method"
                    }];
                    kony.print("success");
...
function invokeEquiWsGetCertList() {
    ...
    if (gvAppNo == null || gvAppNo == "") {
        ...
        if (gblDeviceInfo == "tab") frmCertEquivalencyTab1.show();
        else frmCertEquivalency1.show();
        return;
    }
    showLoadingIndicator(kony.i18n.getLocalizedString("lblPleaseWait"));;
    var inputparam = [];
        ...
        if (status == 400) {
            if ((kony.os.toNumber(result.opstatus) == 0)) {
                ...
                if (studyLevelList[j]["studyLevelID"] == certList[i]["STUDYLEVEL_ID"]) {
                    if (kony.i18n.getCurrentLocale() == "ar_SA") studyLevel = studyLevelList[j]["studyLevel_ar"];
                    else studyLevel = studyLevelList[j]["studyLevel_en"];
                ...
                if (segCertListData.length > 0) frmCertificateEquivalencyLanding.segRegisteredCertificate.setData(segCertListData);
                dismissLoadingIndicator();
            } else {
                dismissLoadingIndicator();
                alertDialog(kony.i18n.getLocalizedString("loadingIndicator"));
...
/*
function invokeEquiWsGetRegCretRelatedDetails() 
{
	gvAppNo = "201412160";
	var inputparam = [];
	inputparam["serviceID"] = "getRegCretRelatedDetails";
	...
	inputparam["APP_NO"] = gvAppNo;
	appmiddlewareinvokerasync(inputparam, equiWsGetRegCretRelatedDetailsCallback);

}*/
//function equiWsGetRegCretRelatedDetailsCallback(status, result) {}
    ...
    try {
        if ((kony.os.toNumber(result.opstatus) == 0)) {
            ...
            if (transcertdetails != null && transcertdetails != "") {
                ...
                if (gblDeviceInfo == "tab") curFrm = frmCertEquivalencyTab2;
                else curFrm = frmCertEquivalency3;
                for (var i = 0; i < transcertdetails.length; i++) {
                    ...
                    if (countriesList[j]["countryID"] != countryId) kony.print("Json countriesList[" + j.toString() + "] " + JSON.stringify(countriesList[j]));
                    else if (countriesList[j]["countryID"] == countryId) {
                        if (kony.i18n.getCurrentLocale() == "ar_SA") country = countriesList[j]["countyName_ar"];
                        else country = countriesList[j]["countyName_en"];
                        break;
                    }
                ...
                if (transSegCertListData.length > 0) curFrm.segTransferCredit.setData(transSegCertListData);
            }
            if (precertdetails != null && precertdetails != "") {
                if (gblDeviceInfo == "tab") curFrm = frmCertEquivalencyTab2;
                else curFrm = frmCertEquivalency4;
                ...
                for (var i = 0; i < precertdetails.length; i++) {
                    ...
                    if (studyLevelList[j]["studyLevelID"] == precertdetails[i]["STUDYLEVEL_ID"]) {
                        if (kony.i18n.getCurrentLocale() == "ar_SA") country = studyLevelList[j]["studyLevel_ar"];
                        else country = studyLevelList[j]["studyLevel_en"];
                        break;
                    }
                    preCertSegListData.push({
                        ...
                        lblCountryYearDisplay: country + ", " + precertdetails[i]["STUDY_FROM"] + "-" + precertdetails[i]["STUDY_TO"],
                        imgPreCertEdit: "edit_on.png",
                        imgPreCertDel: "delete.png",
                        "hdStudyLevelId": precertdetails[i]["STUDYLEVEL_ID"],
                        "hdStudyLevel": studyLevel,
                        "hdCountry": country,
                        "hdCountryId": precertdetails[i]["COUNTRY_ID"],
                ...
                if (preCertSegListData.length > 0) curFrm.segPreviousCertificate.setData(preCertSegListData);
            }
            dismissLoadingIndicator();
        } else {
            dismissLoadingIndicator();
